...
(check-expect (fold string-append "" (list "hey " "hoo " "let's")) "hey hoo let's")

(define (fold fn b lis)
    (cond [(empty? lis) b]
         [else (fn (first lis) (fold fn b (rest lis)))])
 )
...
(define (juxtapose loi) (fold beside (square 0 "solid" "white") loi)) ;stub
...
; Could just return the argument :D
...
;(define (sum-data e) (fold )) ;stub

(define (add-data name value current-value ) (+ value current-value))
(define (sum-data e) (fold-element add-data + 0 e))


;; (String Interger Y -> X) (X Y -> Y) Y Element -> X
...
(define (add-name n d los) (cons n los))
(define (all-names e) (fold-element add-name append empty e))
